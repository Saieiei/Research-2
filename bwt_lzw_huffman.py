...
def dynamic_bwt_lzw_huffman_compression(image_path, chunk_size=1024, bwt_chunk_size=50):
    # Load the image
    ...
    compressed_lzw_data = compress_lzw(compressed_bwt_data)
    
    # Build Huffman tree and code table
    freq_table = build_frequency_table(compressed_lzw_data)
    huffman_tree = build_huffman_tree(freq_table)
    code_table = build_code_table(huffman_tree)
    
    # Compress the LZW data using Huffman coding
    compressed_huffman_data = compress_image(compressed_lzw_data, code_table)
...
def main():
    ...
    # Perform dynamic BWT, LZW, and Huffman compression
    compressed_data = dynamic_bwt_lzw_huffman_compression(image_path, chunk_size, bwt_chunk_size)
    ...
    print("BWT + LZW + Huffman compression:")
